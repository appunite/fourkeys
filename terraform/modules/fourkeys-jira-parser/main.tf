...
resource "google_cloud_run_service" "jira_parser" {
  project  = var.project_id
  name     = "fourkeys-jira-parser"
  location = var.region
...
resource "google_pubsub_topic" "jira" {
  project = var.project_id
  name    = "jira"
}
...
resource "google_pubsub_topic_iam_member" "service_account_editor" {
  project = var.project_id
  topic   = google_pubsub_topic.jira.id
  role    = "roles/editor"
  member  = "serviceAccount:${var.fourkeys_service_account_email}"
}

resource "google_pubsub_subscription" "jira" {
  project = var.project_id
  name    = "jira"
  topic   = google_pubsub_topic.jira.id
  ...
  push_config {
    push_endpoint = google_cloud_run_service.jira_parser.status[0]["url"]
